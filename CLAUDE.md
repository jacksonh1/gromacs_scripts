...
- **REMD/REST2:** `build/ → em/ → heat/ → density/ → equil/ → prod/`

(`heat/` = NVT thermalization at `T_MIN`, restrained, velocities generated there — **all three engines**, `HEAT_NS > 0`; `density/` = iterative NPT density equilibration, each segment resuming from the previous checkpoint and restrained to `em/em.gro`; `relax/` = optional unrestrained NPT, MD-only, `RELAX_NS>0`; `equil/` = per-replica equilibration, REMD/REST2-only.) Do **not** reintroduce ensemble-named folders (`nvt/`, `npt/`, `eq/`). Production basenames stay `md.*`/`remd.*` (method, not ensemble) — the analysis layer keys off them.

### Output model: folder-symlinks into scratch (`SYMLINK_BULK`)

Default (`SYMLINK_BULK=1`): the **small, laptop-worthy** dirs (`build/ em/ analysis/ logs/`, `solvated_snapshots/` (MD only), `parameters.txt`, final PDB) are **real** in `OUTDIR`; the **bulk stage dirs** (`heat/ density/ equil/ prod/` for REMD/REST2; `heat/ density/ [relax/] prod/` for MD; `topol/` stays real for REST2) are **folder symlinks into `SCRATCH_DIR`**, created *before* the run so mdrun writes trajectories/intermediates straight onto scratch. This keeps bulk off the tight-quota pool and makes a laptop rsync of `OUTDIR` drag only a few broken folder-links instead of thousands of per-file links. **Key invariants:** every stage-dir variable (`PROD_DIR`, `DENSITY_DIR`, …) still points at `${OUTDIR}/<stage>` — the symlink is transparent, so **no `deffnm`/`-x`/`grompp` path changes** are needed and analysis reads `prod/…/…​.xtc` through the symlink. `build/` must stay **real** (the `solvate`/`genion` topology `rename()` must stay same-filesystem — see the OUTDIR gotcha). On success a shared `finalize_outputs.sh` copies the real OUTDIR dirs into `SCRATCH_DIR` so the scratch archive is self-contained. There is **no `trajectories/` collection dir** — it was dropped. `SYMLINK_BULK=0` makes every stage dir **real in `OUTDIR`** with no scratch offload (then `SCRATCH_DIR` is empty and all scratch-aware steps skip). When adding a param, keep the three lists in sync (see the STEP-1 validation gotcha); `SYMLINK_BULK` is a shared param (allowlist + `flag01`).

### REMD ensemble toggle (`ENSEMBLE=NVT|NPT`)

T-REMD production runs NVT (default, constant volume) or NPT (`ENSEMBLE=NPT`, C-rescale barostat at `REF_P` bar). **`REF_P`/`TAU_P` are shared params (all three engines) and are NOT "NPT only"** — `density/` runs the barostat under BOTH ensembles because that stage is what determines the box size, so `REF_P` is the pressure the box was equilibrated at regardless of `ENSEMBLE`. `ENSEMBLE` controls only the stages after `density/`: NPT keeps the barostat on in `equil/`+`prod/`, NVT turns it off and freezes the box at whatever `density/` ended on (so `REF_P` still matters under NVT — it sets the pressure that frozen box was equilibrated at). `TAU_P` defaults to 2.0 ps: C-rescale gives the correct NPT distribution at any `tau-p`, so it sets volume relaxation time, not the ensemble — but below ~1 ps it chases virial noise against the `density/` restraints. Under NPT the per-replica `equil/` stage *and* production are pressure-coupled, and GROMACS adds the *PV* term to the replica-exchange Metropolis criterion automatically — `-replex` is unchanged. The constant-temperature ensemble interpretation above is unaffected. Plain MD production is always NPT.
...
- **GROMACS: a density-equilibration segment restarted from a `.gro` loses the barostat's state** — a `.gro` holds only positions/velocities/box, so `grompp -c` without `-t <cpt>` restarts C-rescale cold at every segment boundary and kicks the volume, i.e. the quantity the convergence test measures. Every segment must resume via `-t`.
- **GROMACS: `grompp -r` sets the POSRES reference — passing the previous segment's output lets the restraint ratchet** — a moving reference resists per-segment displacement but never cumulative drift off the designed pose. Anchor every restrained stage to `em/em.gro`.
- **Density convergence must be a plateau (slope) test, not a consecutive-segment difference** — segment-to-segment noise is well under `DENSITY_TOL_REL`, so a sustained drift just under tolerance passes every comparison while the box contracts several percent. `scripts/simulation/density_converged.py` fits the trailing `DENSITY_MIN_SEG` window.
- **GROMACS: `step*.pdb` constraint dumps go to the CWD = the SUBMIT DIR, not `OUTDIR`** — a LINCS failure writes full-system `step<N>b/c.pdb` pairs (75 MB each on a 949k-atom system) next to the submit scripts on the tight-quota pool. Engines now `cd "${SCRATCH_DIR:-$OUTDIR}"` **just before EM** — earlier collides with the cross-fs `solvate`/`genion` `rename()` and kills the build; keep every later path absolute. Usually fires in EM on strained input, not in heat. Since nobody browses scratch, `report_crash_dumps.sh` writes `OUTDIR/CONSTRAINT_FAILURES.txt` + a log banner. It does NOT override `PRESERVE_SCRATCH_FROM` (EM dumps on every run for strained input, so that would disable the knob): policy governs the bulky PDBs, the few-KB report always survives.
- **Output model: folder-symlink stages** — bulk stage dirs (`prod/ equil/ density/ heat/` …) are symlinks into scratch (default `SYMLINK_BULK=1`); never symlink `build/`/`em/` (cross-fs rename), pre-create the stage symlinks before mdrun, and `finalize_outputs.sh` must skip symlinks (`[[ -L ]]`) or it recurses. See the "Output model" contract subsection above.
